...
train_params = {
    ...
    "num_epochs": 2,  # 训练轮次
    "train_batch_size": 10,  # 对于完整yolov3，每一批的训练样本不能太多，内存会炸掉；如果使用tiny，可以适当大一些
...
# 用于展示一张图片用于预测的效果
    ...
    f = open("./input/detection-results/" + image_path[last_slash_index+1:last_dot_index]+'.txt', 'w')
    for i in predict:
        for j in i:
            f.write(str(float(j)) + ' ')
        f.write('\n')
...
if __name__ == '__main__':
    ...
    file_path = os.path.join(train_params['data_dir'], 'eval.txt')
    images = [line.strip() for line in open(file_path)]
    for line in images:
        image_path = line
        parts = line.split('\t')
        filename = parts[0]
        filename_path = os.path.join(train_params['data_dir']+'/lslm_test/', parts[0])
        infer(filename_path)
        ...
        f = open("./input/ground-truth/" + filename_path[24:-4]+'.txt', 'w')
